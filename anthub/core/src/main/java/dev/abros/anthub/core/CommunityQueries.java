...
final class CommunityQueries {
 ...
 JsonArray list(CommunityStore.Actor actor,JsonObject input,String section,long now,JsonObject config)throws Exception {
  ...
  var sources=new ArrayList<String>();for(String source:section.equals("home")?List.of("board","groups","events","polls","ideas"):List.of(section))if(config.getAsJsonArray("sections").contains(new JsonPrimitive(source)))sources.add(source);
  ...
  if(section.equals("home")){
   sql+=" AND (section='board' AND (body->>'owner'=? AND EXISTS (SELECT 1 FROM jsonb_each(body->'responses') r WHERE r.value->>'status'='pending') OR jsonb_exists(body->'responses',?) AND body->'responses'->?->>'status'<>'pending') OR section='groups' AND ((body->>'owner'=? OR body->'members'->>?='assistant') AND body->'applications'<>'{}'::jsonb OR jsonb_exists(body->'invitations',?)) OR section='events' OR section='polls' AND NOT jsonb_exists(body->'votes',?))";
   for(int n=0;n<7;n++)args.add(actor.id());
  }
  ...
  try(var q=db.connection().prepareStatement(sql)){q.setArray(1,db.connection().createArrayOf("text",sources.toArray()));for(int i=0;i<args.size();i++)q.setObject(i+2,args.get(i));try(var rows=q.executeQuery()){while(rows.next()){if(out.size()==10){more=true;break;}var document=Json.parse(rows.getString(1));var preview=CommunityPreview.of(document);if(section.equals("home"))preview.addProperty("attention",switch(Json.str(document,"section")){case "groups"->document.getAsJsonObject("invitations").has(actor.id())?"Вас пригласили":"Заявки ждут ответа";case "board"->Json.str(document,"owner").equals(actor.id())?"Новые отклики":"Есть ответ на ваш отклик";case "polls"->"Вы ещё не голосовали";default->"Ближайшее событие";});preview.addProperty("isParticipant",document.has("participants")&&document.getAsJsonObject("participants").has(actor.id()));preview.addProperty("supported",document.has("supporters")&&document.getAsJsonObject("supporters").has(actor.id()));out.add(preview);last=rows.getLong(2);time=events&&document.has("startsAt")?document.get("startsAt").getAsLong():0;}}}
