...
public final class CommunityStore implements AutoCloseable {
    ...
    Connection connection(){return database.connection();}
    ...
    public String sharedEntry(Actor actor,String section,String id)throws Exception{return database.communityTransaction(()->{require(enabled(section)&&Set.of("board","groups","events","polls","ideas").contains(section));var j=get(id);require(section.equals(str(j,"section"))&&!Set.of("hidden","deleted").contains(str(j,"status")));new CommunityPlus(database,this).requireVisible(actor,j);return str(j,"title");});}
    public void profiles(JsonArray entries)throws Exception{database.communityTransaction(()->{new CommunityPlus(database,this).profiles(entries);return null;});}
    ...
    JsonObject get(String id)throws Exception{return get(id,false);}
    JsonObject get(String id,boolean lock)throws Exception{if(lock)try(var q=connection().prepareStatement("SELECT id FROM documents WHERE id=? FOR UPDATE")){q.setString(1,id);q.executeQuery().close();}try(var q=connection().prepareStatement("SELECT body FROM community_documents WHERE id=?")){q.setString(1,id);try(var rows=q.executeQuery()){if(!rows.next())throw new CommunityFailure(CommunityFailure.Code.NOT_FOUND,"Запись не найдена");return document(rows.getString(1));}}}
    static JsonObject document(String json){var j=JsonParser.parseString(json).getAsJsonObject();if(!j.has("revision")||j.get("revision").isJsonNull())j.addProperty("revision",0);if(j.has("participantOrder")){var ordered=new JsonObject();for(var id:j.getAsJsonArray("participantOrder"))ordered.add(id.getAsString(),j.getAsJsonObject("participants").get(id.getAsString()));j.add("participants",ordered);}return j;}
    void put(JsonObject j)throws Exception{put(j,true);}
    void put(JsonObject j,boolean visible)throws Exception{if(visible)j.addProperty("revision",j.has("revision")?j.get("revision").getAsLong()+1:1);CommunityDocuments.put(database,j);if(visible)CommunityOutbox.add(database,str(j,"section"),str(j,"id"),"");}
    ...
    void note(String recipient,String section,String target,String title)throws Exception{
    ...
    JsonObject publicView(JsonObject source,Actor a,int page)throws Exception{var j=source.deepCopy();j.add("actions",CommunityPolicy.actions(source,a,System.currentTimeMillis()));String section=str(j,"section");j.addProperty("manage",owner(j,a)||a.admin);
    ...
    public JsonObject request(Actor a,JsonObject input)throws Exception{return database.communityTransaction(()->{
        ...
        if(op.equals("plusCancelSeries"))database.lock("community:event-series");
        ...
        var receipt=new RequestJournal(database).execute(a.id,receiptInput(a,input),()->{JsonObject result=execute(a,input,op,section);if(!Set.of("list","detail","read","plusProfile","plusIgnores","plusItemRead").contains(op))audit(a.name,"community "+op,section+" "+value(input,"id"));return result;});
        if(receipt.has("replayed")&&op.startsWith("plus"))return receipt;
    ...
    JsonObject execute(Actor a,JsonObject in,String op,String section)throws Exception{
        long now=System.currentTimeMillis();String id=value(in,"id");
        var plus=new CommunityPlus(database,this);if(op.startsWith("plus"))return plus.request(a,in,section);
        if(op.equals("preferences")){var p=in.getAsJsonObject("preferences");Json.keys(p,"favorites","muted");for(String k:List.of("muted")){if(!p.has(k)||p.getAsJsonArray(k).size()>10)throw new IllegalArgumentException("Too many settings");for(var v:p.getAsJsonArray(k))require(SECTIONS.contains(v.getAsString()));}try(var s=connection().prepareStatement("INSERT INTO preferences VALUES(?,?::jsonb) ON CONFLICT(id) DO UPDATE SET body=excluded.body")){s.setString(1,a.id);var clean=p.deepCopy();clean.remove("favorites");s.setString(2,Json.GSON.toJson(clean));s.executeUpdate();}}
            ...
            }if(Set.of("board","events").contains(section)&&!value(in,"group").isEmpty()){var group=get(value(in,"group"));require(str(group,"section").equals("groups")&&str(group,"status").equals("open")&&contains(group,"members",a.id));j.addProperty("group",str(group,"id"));j.addProperty("groupName",str(group,"title"));}if(in.has("location")&&!in.get("location").isJsonNull())setLocation(j,in);plus.prepare(a,in,j);EntryHistory.append(j,op,a.name,now);put(j);plus.repeat(a,in,j);
        }else if(!op.equals("list")&&!op.equals("detail")){
            var j=get(id,true);plus.requireVisible(a,j);require(section.equals(str(j,"section")));require(!str(j,"status").equals("hidden")||a.admin||owner(j,a));String title=str(j,"title");boolean own=owner(j,a);CommunityPolicy.check(j,a,op,now);
            ...
            if(op.equals("delete")){j.addProperty("previousStatus",str(j,"status"));j.addProperty("status","deleted");j.addProperty("deletedAt",now);j.addProperty("deletedBy",a.id);
                ...
                if(section.equals("groups"))for(var candidate:bodies("SELECT body FROM community_documents WHERE body->>'group'=? ORDER BY id",id)){var child=get(str(candidate,"id"),true);if(str(child,"section").equals("events")&&!Json.opt(child,"visibility","public").equals("public")){child.addProperty("status","cancelled");child.addProperty("visibility","invited");}child.remove("group");child.remove("groupName");put(child);}
                ...
                switch(op){
                    ...
                    case "reschedule" -> {require(own||a.admin);long start=Instant.parse(text(in,"startsAt",40)).toEpochMilli();require(start>now);j.addProperty("startsAt",start);j.addProperty("rescheduledAt",now);j.add("reminded",new JsonObject());for(String target:map(j,"participants").keySet())note(target,section,id,"Событие перенесено: "+title);}
                ...
                switch(op){
                    ...
                    case "invite" -> {require(manager(j,a)&&!members.has(target));UUID.fromString(target);plus.allowContact(target,a.id);boundedMap(j,"invitations",target);require(!contains(j,"invitations",target));map(j,"invitations").addProperty(target,true);note(target,section,id,"Приглашение: "+title);}
    ...
    private static Set<String> groupPeople(JsonObject detail){var ids=new HashSet<String>();for(String key:List.of("members","applications","invitations"))ids.addAll(map(detail,key).keySet());if(detail.has("groupItems"))for(var item:detail.getAsJsonArray("groupItems")){String assignee=Json.opt(item.getAsJsonObject(),"assignee","");if(!assignee.isBlank())ids.add(assignee);}return ids;}
    boolean manager(JsonObject j,Actor a){return a.admin||owner(j,a)||contains(j,"members",a.id)&&map(j,"members").get(a.id).getAsString().equals("assistant");}
    private int memberships(String id)throws Exception{try(var q=connection().prepareStatement("SELECT count(*) FROM community_documents WHERE section='groups' AND body->>'status'='open' AND jsonb_exists(body->'members',?)")){q.setString(1,id);try(var r=q.executeQuery()){r.next();return r.getInt(1);}}}
    ...
    public void reminders(long now)throws Exception{database.communityTransaction(()->{database.lock("community:reminders");if(lastReminderSweep!=0&&now-lastReminderSweep<30000)return null;try{for(var candidate:due("events",now)){var j=get(str(candidate,"id"),true);long start=time(j,"startsAt");if(!str(j,"status").equals("open"))continue;boolean changed=false;var targets=new LinkedHashSet<>(map(j,"participants").keySet());targets.add(str(j,"owner"));try(var q=connection().prepareStatement("SELECT player FROM community_event_preferences WHERE event=? AND minutes>=0")){q.setString(1,str(j,"id"));try(var rows=q.executeQuery()){while(rows.next())targets.add(rows.getString(1));}}for(String target:targets){if(!new CommunityPlus(database,this).visible(new Actor(target,"",false,false),j))continue;int minutes=5;try(var q=connection().prepareStatement("SELECT minutes FROM community_event_preferences WHERE event=? AND player=?")){q.setString(1,str(j,"id"));q.setString(2,target);try(var rows=q.executeQuery()){if(rows.next())minutes=rows.getInt(1);}}if(minutes<0||now<start-minutes*60000L||now>start+60000)continue;if(!contains(j,"reminded",target)){note(target,"events",str(j,"id"),(start<=now?"Начинается событие: ":"Скоро событие: ")+str(j,"title"));map(j,"reminded").addProperty(target,true);changed=true;}}if(changed)put(j,false);}for(var candidate:due("polls",now)){var j=get(str(candidate,"id"),true);if(time(j,"endsAt")<=now&&!j.has("notified")){for(String target:map(j,"votes").keySet())note(target,"polls",str(j,"id"),"Голосование завершено: "+str(j,"title"));j.addProperty("notified",true);put(j,false);}}lastReminderSweep=now;}catch(Exception ex){lastReminderSweep=0;throw ex;}return null;});}
    ...
    private List<JsonObject> memberGroups(String id)throws Exception{
        return bodies("SELECT body FROM community_documents WHERE section='groups' AND body->>'status'='open' AND jsonb_exists(body->'members',?) ORDER BY sequence DESC",id);
    }
    private List<JsonObject> related(String section,String group)throws Exception{
        return bodies("SELECT body FROM community_documents WHERE section=? AND body->>'group'=? AND body->>'status' NOT IN ('hidden','deleted') ORDER BY sequence DESC LIMIT 20",section,group);
    }
    private List<JsonObject> due(String section,long now)throws Exception{
        ...
        return section.equals("events")?bodies("SELECT body FROM community_documents WHERE section='events' AND "+predicate+" ORDER BY sequence",now-60000,now+3600000):bodies("SELECT body FROM community_documents WHERE section='polls' AND "+predicate+" ORDER BY sequence",now);
    }
