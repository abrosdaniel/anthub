...
public final class CommunityPreview {
 ...
 public static JsonObject of(JsonObject document){
  var row=new JsonObject();for(String key:List.of("id","section","title","author","status","createdAt","startsAt","endsAt","type","recruiting","capacity","revision"))if(document.has(key))row.add(key,document.get(key).deepCopy());
