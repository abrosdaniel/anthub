...
public final class TestDatabase {
 ...
 public static synchronized PgDatabase database(Path identity)throws Exception{
  ...
  if(!identity.equals(test)){database.transaction(()->{try(var statement=database.connection().createStatement()){statement.execute("TRUNCATE community_events,request_receipts,documents,people,notices,records,preferences,auth_devices,auth_resets,auth_accounts,auth_failures,auth_audit RESTART IDENTITY CASCADE");}return null;});test=identity;}
  return database;
 }
}
