...
class CommunityStoreTest {
 ...
 @Test void participationFiltersAndHomeShowRelevantEntries()throws Exception{
  ...
  var ownFeed=db.request(owner,input("home","list","")).getAsJsonArray("entries");assertEquals(id(group),id(ownFeed.get(0).getAsJsonObject()));
  ...
  var reply=input("board","respond",id(post));reply.addProperty("text","Help");db.request(alice,reply);assertEquals(2,db.request(owner,input("home","list","")).getAsJsonArray("entries").size());
 }
 ...
 @Test void homeShowsUpcomingEventsBeforeJoining()throws Exception{var event=create("events");var feed=db.request(alice,input("home","list","")).getAsJsonArray("entries");assertTrue(java.util.stream.StreamSupport.stream(feed.spliterator(),false).anyMatch(e->id(e.getAsJsonObject()).equals(id(event))));}
