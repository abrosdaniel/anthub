...
class ManifestTest {
    ...
    @Test void unchangedFilesActivateAndUpdateWithoutRestart()throws Exception{
        ...
        var hub=new Hub(game,"1.0.0-alpha.1","21.1.250");var release=new RepositoryClient.Release(manifest,Json.GSON.toJson(json) .getBytes(),false,false,"");
        ...
        assertEquals("",hub.stage(next,hub.plan(next,Set.of()),new java.util.concurrent.atomic.AtomicBoolean(),s->{}));assertEquals("1.0.1",new Hub(game,"1.0.0-alpha.1","21.1.250").active().version());assertFalse(Files.exists(game.resolve("anthub/pending.json")));assertArrayEquals(content,Files.readAllBytes(path));
    }
    ...
    @Test void removalWaitsUntilProjectIsDeactivated()throws Exception{
        var manifest=Manifest.parse(fixture());var hub=new Hub(game,"1.0.0-alpha.1","21.1.250");hub.rememberProject(manifest);var state=new JsonObject();state.add("lock",manifest.json());Json.write(game.resolve("anthub/state.json"),state);hub.removeAfterDeactivation(manifest.repository());
        assertTrue(new Hub(game,"1.0.0-alpha.1","21.1.250").saved().contains(manifest.repository()));
        Files.writeString(game.resolve("personal.txt"),"keep");Json.write(game.resolve("anthub/state.json"),new JsonObject());var reopened=new Hub(game,"1.0.0-alpha.1","21.1.250");assertFalse(reopened.saved().contains(manifest.repository()));assertEquals("keep",Files.readString(game.resolve("personal.txt")));
    }
}
