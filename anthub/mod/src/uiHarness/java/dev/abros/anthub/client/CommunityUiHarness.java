 ...
 @SubscribeEvent public static void smoke(ScreenEvent.Render.Post event){
  ...
  try{
   ...
   switch(smokeStep++){
    ...
    default -> {System.out.println("ANTHUB_UI_SMOKE_COMPLETE");if(System.getenv("ANTHUB_UI_KEEP_OPEN")!=null){smokeStep=-1;mc.options.guiScale().set(2);mc.resizeDisplay();open("normal");mc.setScreen(new CommunityScreen(new TitleScreen(),"groups",""));}else mc.stop();}
   }
