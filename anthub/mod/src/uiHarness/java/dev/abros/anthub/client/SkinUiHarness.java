...
public final class SkinUiHarness {
 ...
 @SubscribeEvent public static void tick(ClientTickEvent.Post event){String output=System.getenv("ANTHUB_SKIN_SMOKE");if(output==null||step<0)return;var mc=Minecraft.getInstance();long now=System.currentTimeMillis();if(now<next)return;
  try{if(deadline==0)deadline=now+150000;if(now>deadline)throw new IllegalStateException("Skin smoke timeout, step "+step+" status "+SkinClient.status);switch(step){
   ...
   case 1->{if(mc.player==null||!SkinClient.available())return;if(System.getenv("ANTHUB_VOTE_NOTICE_TEST")!=null){if(!Json.opt(ServerMenuClient.moderationVote,"status","").equals("open"))return;var field=NoticeToast.class.getDeclaredField("ID");field.setAccessible(true);if(mc.getToasts().getToast(NoticeToast.class,field.get(null))==null)return;System.out.println("ANTHUB_LATE_JOIN_VOTE_NOTICE_OK");}next=now+1500;step++;}
   ...
   case 5->{if(!mc.player.getSkin().texture().getNamespace().equals("anthub"))return;checkSkinPaths(mc,true);var dir=new java.io.File(output);dir.mkdirs();net.minecraft.client.Screenshot.grab(dir,"skin-wide.png",mc.getMainRenderTarget(),m->{});SkinClient.command("model",id,true);step++;}
