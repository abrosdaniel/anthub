...
final class MenuSidebar {
 ...
 void build(String current,Consumer<Button> add){var keys=new ArrayList<String>();var config=ServerMenuClient.state.has("communityConfig")?ServerMenuClient.state.getAsJsonObject("communityConfig"):new JsonObject();for(String key:List.of("home","players","board","groups","events","polls","ideas","info","help","admin")){if(ServerMenuClient.state.has("features")&&!ServerMenuClient.state.getAsJsonArray("features").contains(new JsonPrimitive(key)))continue;if(key.equals("admin")&&!ServerMenuClient.staff())continue;if(config.has("sections")&&!Set.of("info","help","admin").contains(key)&&!config.getAsJsonArray("sections").contains(new JsonPrimitive(key)))continue;keys.add(key);}int shown=Math.max(1,(host.height-82)/24);offset=Math.max(0,Math.min(offset,Math.max(0,keys.size()-shown)));for(int i=offset;i<Math.min(keys.size(),offset+shown);i++){String key=keys.get(i);String label=Minecraft.getInstance().font.plainSubstrByWidth(CommunityScreen.name(key),left()-28);var button=new SidebarButton(8,42+(i-offset)*24,left()-20,label,current.equals(key),()->navigate(key));add.accept(button);}if(!ServerMenuClient.state.has("features")||ServerMenuClient.state.getAsJsonArray("features").contains(new JsonPrimitive("notifications")))add.accept(Button.builder(Component.literal("Уведомления"),b->Minecraft.getInstance().setScreen(new NotificationPopup(host))).bounds(8,host.height-28,left()-20,20).build());}
