...
public final class ServerMenuClient {
    ...
    private static void tick(net.neoforged.neoforge.client.event.ClientTickEvent.Post e){XaeroMapBridge.tick();
        ...
        if(current!=connection){connection=current;state=new JsonObject();moderationVote=new JsonObject();result="";Protocol.profile=new JsonObject();lastRequest=0;stateRequested=false;lastPopup=0;CommunityScreen.clearDrafts();subscription="";subscriptionAt=0;outbound.clear();dispatched.clear();}
    ...
    static void receive(JsonObject j){
        ...
        if(kind.equals("changed")){if(mc.screen instanceof CommunityScreen screen)screen.invalidate(Json.opt(j,"section",""),Json.opt(j,"id",""));else if(mc.screen instanceof FeatureListScreen screen)screen.invalidate();else if(mc.screen instanceof NotificationPopup screen)screen.invalidate();else if(mc.screen instanceof ChoicePopup popup)popup.invalidate();return;}
