...
final class AuthAccountScreen extends ScrollScreen {
    ...
    private int panelTop(){return DialogPanel.top(height,300);}
 private int panelBottom(){return height-panelTop();}
 @Override public void renderBackground(GuiGraphics g,int x,int y,float d){super.renderBackground(g,x,y,d);DialogPanel.draw(g,width,Math.min(460,width-40),panelTop(),panelBottom());}
 @Override protected void init(){int w=Math.min(460,width-40),x=(width-w)/2;
        if(invite!=null){var code=addRenderableWidget(new EditBox(font,x,panelTop()+90,w,20,Component.literal("Код восстановления")));code.setMaxLength(128);code.setValue(Json.str(invite,"token"));code.setEditable(false);addRenderableWidget(Button.builder(Component.literal("Скопировать код"),b->{if(System.currentTimeMillis()<invite.get("expires").getAsLong()){minecraft.keyboardHandler.setClipboard(Json.str(invite,"token"));status="Скопировано. Передайте владельцу лично";}else status="Приглашение истекло";}).bounds(x,panelTop()+118,w,20).build());}
        ...
        else if(changing){oldPassword=secret(x,55,w,"Текущий пароль");oldPassword.setHint(Component.literal("Текущий пароль"));password=secret(x,85,w,"Новый пароль");password.setHint(Component.literal("Новый пароль · от "+AuthClient.minimumPasswordLength()+" символов"));repeat=secret(x,115,w,"Повтор нового пароля");repeat.setHint(Component.literal("Повтор нового пароля"));addRenderableWidget(Button.builder(Component.literal("Изменить пароль и отозвать все сеансы"),b->{if(password.getValue().length()<AuthClient.minimumPasswordLength()){status="Нужно не менее "+AuthClient.minimumPasswordLength()+" символов";return;}if(!password.getValue().equals(repeat.getValue())){status="Пароли не совпадают";return;}var j=new JsonObject();j.addProperty("action","change");j.addProperty("oldPassword",oldPassword.getValue());j.addProperty("password",password.getValue());oldPassword.setValue("");password.setValue("");repeat.setValue("");AuthClient.send(j);status="Проверка…";}).bounds(x,panelTop()+145,w,20).build());setInitialFocus(oldPassword);}
        else {scrollArea(devices.size(),panelTop()+50,panelBottom()-140,36,x+w+4);for(int i=firstRow;i<Math.min(devices.size(),firstRow+visibleRows);i++){var device=devices.get(i).getAsJsonObject();addRenderableWidget(Button.builder(Component.literal(Json.str(device,"label")+(Json.str(device,"id").equals(currentDevice)?" (это устройство)":"")+" · Отозвать"),b->revoke(Json.str(device,"id"))).bounds(x,panelTop()+50+(i-firstRow)*36,w,20).build());}
            if(mode.equals("hybrid")){var link=addRenderableWidget(Button.builder(Component.literal(linked?"Отвязать Minecraft-аккаунт":"Привязать Minecraft-аккаунт"),b->{linkAction=linked?"unlink":"link";status="Подтвердите действие паролем сервера";rebuildWidgets();}).bounds(x,panelBottom()-128,w,20).build());link.active=linked||AuthClient.officialLauncher();}
            var change=addRenderableWidget(Button.builder(Component.literal("Сменить пароль"),b->{changing=true;rebuildWidgets();}).bounds(x,panelBottom()-100,w,20).build());change.active=type.equals("local");addRenderableWidget(Button.builder(Component.literal("Отозвать все устройства и сеансы"),b->revoke("all")).bounds(x,panelBottom()-76,w,20).build());}
        addRenderableWidget(Button.builder(Component.literal("Назад"),b->onClose()).bounds(x,panelBottom()-26,w,20).build());
    }
    ...
    @Override public void render(GuiGraphics g,int x,int y,float delta){super.render(g,x,y,delta);g.drawCenteredString(font,title,width/2,panelTop()+18,0xE2BE75);if(invite!=null){g.drawCenteredString(font,"Сброс доступа: "+Json.str(invite,"target"),width/2,panelTop()+50,0xFFFFFF);g.drawCenteredString(font,"Осталось секунд: "+Math.max(0,(invite.get("expires").getAsLong()-System.currentTimeMillis())/1000),width/2,panelTop()+70,0xAAAAAA);}Ui.status(g,font,status,(width-Math.min(460,width-40))/2,panelBottom()-52,Math.min(460,width-40),panelBottom()-28);}
}
