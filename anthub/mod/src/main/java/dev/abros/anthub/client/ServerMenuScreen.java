...
final class ServerMenuScreen extends ScrollScreen {
    ...
    @Override protected void init(){
        ...
        if(tab.equals("admin")){
            ...
            int count=adminGroup.equals("server")?(ServerMenuClient.supports("admin-tools")?5:3):1;scrollArea(count,76,height-64,56,width-10);
            for(int row=firstRow;row<Math.min(count,firstRow+visibleRows);row++){
                ...
                switch(row){
                    ...
                    case 3 -> {addRenderableWidget(Button.builder(Component.literal("Закрепить сообщение"),b->action("pinAnnouncement")).bounds(left+6,y,half,20).build());addRenderableWidget(Button.builder(Component.literal("Снять закрепление"),b->{var j=new JsonObject();j.addProperty("action","pinAnnouncement");j.addProperty("text","");j.addProperty("minutes",1);ServerMenuClient.request(j);}).bounds(left+half+10,y,half,20).build());}
                    case 4 -> addRenderableWidget(Button.builder(Component.literal("Экспорт сообщества"),b->ServerMenuClient.request("exportCommunity")).bounds(left+6,y,half,20).build());
                }
            }
        }
    ...
    @Override public void renderBackground(GuiGraphics g,int x,int y,float d){super.renderBackground(g,x,y,d);if(tab.equals("admin")){
        String[] labels=adminGroup.equals("reports")?new String[]{"Очередь обращений и ответы игрокам"}:adminGroup.equals("journal")?new String[]{"История действий и состояние сервера"}:new String[]{"Объявления для игроков","Техническое обслуживание","Перезапуск с обратным отсчётом","Настройки меню","Закреплённое объявление","Экспорт объявлений, объединений и событий"};
        for(int row=firstRow;row<Math.min(labels.length,firstRow+visibleRows);row++){int top=76+(row-firstRow)*56;g.fill(nav.left(),top,width-12,top+50,AccessibilityScreen.background(0xCE25232A));g.fill(nav.left(),top,nav.left()+3,top+50,AccessibilityScreen.background(0xFFE2BE75));g.drawString(font,font.plainSubstrByWidth(labels[row],width-nav.left()-26),nav.left()+8,top+7,0xE2BE75);}
    }}
    ...
    private static final class ActionForm extends Screen {
        ...
        @Override protected void init(){
            ...
            if(timed()){duration=addRenderableWidget(new EditBox(font,x,108,100,20,Client.tr(!action.equals("restart")?"server.minutes":"server.seconds")));duration.setMaxLength(5);duration.setFilter(v->v.matches("[0-9]*"));duration.setValue(value);}
        ...
        private void submit(){
            ...
            Component summary=Component.literal(message.getValue());if(timed())summary=summary.copy().append(" · "+amount+" ").append(Client.tr(!action.equals("restart")?"server.minutes":"server.seconds"));
        ...
        @Override public void render(GuiGraphics g,int x,int y,float d){super.render(g,x,y,d);int left=(width-Math.min(420,width-40))/2;
            ...
            if(timed())g.drawString(font,Client.tr(!action.equals("restart")?"server.minutes":"server.seconds"),left,94,AccessibilityScreen.foreground(0xEEEEEE));
