...
final class ServerMenuScreen extends ScrollScreen {
    private final MenuSidebar nav=new MenuSidebar(this);private final Screen parent;private String tab="help";private final ContentPane content=new ContentPane();
    ...
    @Override protected void init(){
        ...
        if(tab.equals("admin")){
            scrollArea(5,50,height-60,56,width-10);
            for(int row=firstRow;row<Math.min(5,firstRow+visibleRows);row++){
                int y=50+(row-firstRow)*56+22,half=(width-nav.left()-22)/2;
                switch(row){
                    case 0 -> button("server.announce",nav.left()+6,y,half,()->action("announce"));
                    case 1 -> {button("server.maintenance",nav.left()+6,y,half,()->action("maintenance"));button("server.endmaintenance",nav.left()+half+10,y,half,()->action("maintenanceOff"));}
                    case 2 -> {button("server.restart",nav.left()+6,y,half,()->action("restart"));button("server.cancelrestart",nav.left()+half+10,y,half,()->action("restartOff"));}
                    case 3 -> {button("server.reports",nav.left()+6,y,half,()->FeatureListScreen.open(this,"reports"));button("server.history",nav.left()+half+10,y,half,()->FeatureListScreen.open(this,"history"));}
                    case 4 -> {button("server.diagnostics",nav.left()+6,y,half,()->ServerMenuClient.request("diagnostics"));button("server.reloadMenu",nav.left()+half+10,y,half,()->ServerMenuClient.request("reloadMenu"));}
                }
            }
        }
    ...
    @Override public void renderBackground(GuiGraphics g,int x,int y,float d){super.renderBackground(g,x,y,d);if(tab.equals("admin")){
        String[] labels={"Объявления для игроков","Техническое обслуживание","Перезапуск сервера","Обращения и журнал","Диагностика и настройки"};
        for(int row=firstRow;row<Math.min(5,firstRow+visibleRows);row++){int top=50+(row-firstRow)*56;g.fill(nav.left(),top,width-12,top+50,0xCE25232A);g.fill(nav.left(),top,nav.left()+3,top+50,0xFFE2BE75);g.drawString(font,font.plainSubstrByWidth(labels[row],width-nav.left()-26),nav.left()+8,top+7,0xE2BE75);}
    }}
