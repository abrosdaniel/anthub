...
final class FeatureListScreen extends ScrollScreen {
    ...
    @Override public void tick(){if(!ServerMenuClient.available())minecraft.setScreen(null);else if(busy&&System.currentTimeMillis()-lastLoad>15000)failure("Нет ответа. Нажмите «Обновить».");else if(!busy&&nextAt>0&&System.currentTimeMillis()>=nextAt){nextAt=0;load(page+1);}else if(!busy&&dirtyAt>0&&System.currentTimeMillis()-lastLoad>750){dirtyAt=0;refresh();}else if((kind.equals("reports")&&!ServerMenuClient.may("anthub.reports")||kind.equals("history")&&!ServerMenuClient.admin()))minecraft.setScreen(parent);}
