...
final class ProfilePanel {
 ...
 static void render(GuiGraphics g,Font font,int x,int y,int w,boolean full,com.google.gson.JsonObject data){
  ...
  g.fill(x,y,x+w,y+(full?192:66),AccessibilityScreen.background(0xDD1C2731));g.fill(x,y,x+3,y+(full?192:66),AccessibilityScreen.background(0xFFE2BE75));
  ...
  if(full){if(data.has("selfStatistics")&&data.getAsJsonObject("selfStatistics").has("totalMillis")){long minutes=data.getAsJsonObject("selfStatistics").get("totalMillis").getAsLong()/60000;g.drawString(font,"Всего: "+minutes/60+" ч "+minutes%60+" мин",x+9,y+80,AccessibilityScreen.foreground(0xBAC7D2));}String seconds=DisplayCounts.text(state,"sessionSeconds","0");if(state.has("sessionSeconds"))g.drawString(font,"В игре: "+Long.parseLong(seconds)/60+" мин.",x+9,y+96,AccessibilityScreen.foreground(0xBAC7D2));g.drawString(font,"Онлайн: "+DisplayCounts.text(state,"online","—")+" / "+DisplayCounts.text(state,"maximum","—"),x+9,y+110,AccessibilityScreen.foreground(0xBAC7D2));g.drawString(font,AuthClient.available()?"AntHub Auth · вход выполнен":"Вход через Minecraft",x+9,y+124,0x79CBA6);}
  g.disableScissor();
 }
}
