...
final class SkinsScreen extends ScrollScreen {
 ...
 @Override public void render(GuiGraphics g,int mx,int my,float delta){super.render(g,mx,my,delta);g.drawCenteredString(font,title,width/2,18,0xE2BE75);if(draft!=null)g.drawString(font,"Название",left+6,46,0xFFFFFF);else for(int row=firstRow;row<Math.min(entries().size()+1,firstRow+visibleRows);row++){String id=row==0?"":Json.str(entries().get(row-1).getAsJsonObject(),"id");String label=row==0?"Обычный скин":Json.str(entries().get(row-1).getAsJsonObject(),"name");var face=DefaultPlayerSkin.get(minecraft.player==null?new java.util.UUID(0,0):minecraft.player.getUUID());if(row>0){var entry=entries().get(row-1).getAsJsonObject();var texture=SkinClient.texture(Json.str(entry,"hash"));if(texture!=null)face=new PlayerSkin(texture,null,null,null,entry.get("slim").getAsBoolean()?PlayerSkin.Model.SLIM:PlayerSkin.Model.WIDE,false);}PlayerFaceRenderer.draw(g,face,left+7,47+(row-firstRow)*72,16);g.drawString(font,font.plainSubstrByWidth(label,listWidth-36),left+28,50+(row-firstRow)*72,0xFFFFFF);if(id.equals(selection))g.renderOutline(left+1,44+(row-firstRow)*72,listWidth-2,70,0xFFE2BE75);}
