...
final class ReviewScreen extends TextScreen {
 ...
 @Override protected void init(){super.init();clearWidgets();addRenderableWidget(Button.builder(Client.tr("cancel"),b->onClose()).bounds(width/2-150,height-26,100,20).build());addRenderableWidget(Button.builder(action,b->confirm.run()).bounds(width/2-44,height-26,194,20).build());}
}
