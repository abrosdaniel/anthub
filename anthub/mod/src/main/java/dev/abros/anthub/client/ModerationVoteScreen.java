...
final class ModerationVoteScreen extends ScrollScreen implements CommunityScreen.Receiver {
 private final Screen parent;private JsonObject target,vote=new JsonObject();private boolean creating,busy,started;private String minutes="30";private String reason="",measure="kick",status="",request="";private long sent,lastRefresh;private final List<String> lines=new ArrayList<>();
 ...
 @Override protected void init(){if(!customDuration())minutes=setting(measure.equals("mute")?"muteMinutes":"banMinutes",measure.equals("mute")?"15":"30");int top=formTop();int w=Math.min(480,width-28),left=(width-w)/2;lines.clear();
  if(creating){
  ...
  addRenderableWidget(Button.builder(Client.tr("back"),b->onClose()).bounds(creating?left+w/2+3:left,creating?top+184:height-28,creating?w/2-3:w,20).build());if(!started){started=true;if(!creating)send("view",new JsonObject());}
 }
