...
public final class SkinClient {
 ...
 private static void receive(JsonObject j){try{switch(Json.str(j,"kind")){
  ...
  case "error"->{if(session.receive(j)){busy=false;retryable=false;uploadBytes=null;outgoing.removeIf(p->pending.equals(Json.opt(p,"request","")));status=Json.opt(j,"text","Ошибка загрузки");changed();}}
