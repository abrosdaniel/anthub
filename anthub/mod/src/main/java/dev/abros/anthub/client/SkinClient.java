...
public final class SkinClient {
 ...
 static void command(String op,String id,boolean slim,String name){if(busy)return;var j=new JsonObject();j.addProperty("op",op);j.addProperty("name",name);j.addProperty("id",id);j.addProperty("slim",slim);pending=UUID.randomUUID().toString();j.addProperty("request",pending);busy=true;sentAt=System.currentTimeMillis();status="";send(j);}
