...
SCHEMAS=Path(__file__).resolve().parents[2]/'core/src/main/resources/anthub/schemas/v1'
def pairs(items):
    d={}
    for k,v in items:
...
def load_project(root):
    ...
    p['anthub']={'version':p.pop('anthubVersion')}
...
def build(args):
    ...
    (out/'anthub.lock.sha256').write_text(sha(data)+'\n')
...
def verify_release(folder):
    ...
    if sha(raw)!=(folder/'anthub.lock.sha256').read_text().strip():raise ValueError('Lock hash mismatch')
